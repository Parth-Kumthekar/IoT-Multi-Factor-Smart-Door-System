 ...
 * FIX: Added [[maybe_unused]] to silence compiler warnings.
 */
size_t write_data([[maybe_unused]] void *buffer, size_t size, size_t nmemb, [[maybe_unused]] void *userp) {
    return size * nmemb;
}
...
/**
 * @brief Thread-safe log submission.
 */
...
/**
 * @brief Consumer Loop with fixed resource management.
 */
void AsyncLogger::processLogs() {
    ...
    while (true) {
        ...
        struct tm timeinfo;
        localtime_r(&time_t_now, &timeinfo); // Thread-safe version of localtime
        ...
        if (curl) {
            ...
            // Clean up headers after each request to prevent memory leak
