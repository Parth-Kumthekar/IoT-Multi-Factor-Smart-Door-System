...
AsyncLoggerCam::AsyncLoggerCam(const std::string& csvPath) : csvPath_(csvPath) {
    ...
    // THREAD
    worker_ = std::thread(&AsyncLoggerCam::workerLoop, this);
}

AsyncLoggerCam::~AsyncLoggerCam() {
    queue_.shutdown();               
    if (worker_.joinable())
        worker_.join();              
    csv_.flush();
...
void AsyncLoggerCam::log(AccessEvent ev) {
    queue_.push(std::move(ev));      
}

void AsyncLoggerCam::workerLoop() {
    while (true) {
        auto ev = queue_.pop();      
        if (!ev) break;              
...
const std::vector<AccessEvent>& AsyncLoggerCam::entries() const {
    return entries_; 
}
