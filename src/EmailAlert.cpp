...
namespace {
    /**
     * @struct UploadStatus
     * @brief Internal state tracker for the libcurl read callback.
     */
    struct UploadStatus {
        std::size_t bytesRead = 0; ///< Offset of the data already sent.
        std::string payload;       ///< The full SMTP formatted email string.
    };

    /**
     * @brief Callback function used by libcurl to read the email payload.
     * @details This function is called repeatedly by curl to upload the email data
     * until 0 is returned.
     * @return std::size_t The number of bytes copied into the buffer.
     */
    ...
    /**
     * @brief Constructs a raw SMTP formatted string.
     * @details includes the Date, To, From, and Subject headers followed by the body.
     * @return std::string The formatted email ready for transmission.
     */
...
/**
 * @brief Default Constructor for Team 22.
 * @details Initializes SMTP settings for Gmail by default. 
 * @note Requires a valid Google App Password to function with MFA-enabled accounts.
 */
...
/**
 * @brief Custom Constructor for arbitrary SMTP servers.
 */
...
/**
 * @brief Dispatches an email using the libcurl easy interface.
 * @details Handles the SMTP handshake, TLS encryption (STARTTLS), and data upload.
 * @param subject The email subject line.
 * @param body The message content.
 * @return true If the SMTP server accepted the message (CURLE_OK).
 * @return false If connection, authentication, or transmission failed.
 */
